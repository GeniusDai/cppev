...
TEST_P(TestNioSocket, test_tcp_socket) {
    ...
    ASSERT_EQ(sock->get_so_reuseaddr(), std::get<1>(p));
    ASSERT_EQ(sock->get_so_reuseport(), std::get<1>(p));
    ASSERT_EQ(sock->get_so_keepalive(), std::get<1>(p));
    ASSERT_EQ(sock->get_tcp_nodelay(), std::get<1>(p));
    auto lg = sock->get_so_linger();
    ASSERT_EQ(lg.first, std::get<1>(p));
    if (lg.first) { ASSERT_EQ(lg.second, std::get<2>(p)); }
    ...
    ASSERT_EQ(sock->get_so_rcvbuf(), std::get<2>(p));
    ASSERT_EQ(sock->get_so_sndbuf(), std::get<2>(p));
    ASSERT_EQ(sock->get_so_sndlowat(), std::get<2>(p));
    ASSERT_EQ(sock->get_so_rcvlowat(), std::get<2>(p));
}
...
TEST_P(TestNioSocket, test_udp_socket) {
    ...
    ASSERT_EQ(sock->get_so_reuseaddr(), std::get<1>(p));
    ASSERT_EQ(sock->get_so_reuseport(), std::get<1>(p));
    ASSERT_EQ(sock->get_so_broadcast(), std::get<1>(p));
